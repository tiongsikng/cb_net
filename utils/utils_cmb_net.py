...
class OnlineContrastiveLoss(nn.Module):
    ...
    def forward(self, embeddings, target):
        ...
        if embeddings.is_cuda:
            positive_pairs = positive_pairs.to(params.device)
            negative_pairs = negative_pairs.to(params.device)
...
class OnlineTripletLoss(nn.Module):
    ...
    def forward(self, embeddings, target, target_min = 0, k = 1, ap_flag = False):
        ...
        ap = ap_distances.mean().cpu()
        an = an_distances.mean().cpu()
    ...
    def get_triplets(self, embeddings, labels, label_min = 0, k = 1):
        ...
        for label in set(labels):
            ...
            if label >= label_min:
                ...
                # permutations
                # perms = list(permutations(label_indices, 2))
                # skip_step = int(len(perms) / len(label_indices))
                # anchor_positives = perms[::skip_step]
                # combinations
                # anchor_positives = list(combinations(label_indices, 2))
                ...
                # ap_distances = ((anchor_positives[:, 0] - anchor_positives[:, 1])**2)
                
                for anchor_positive, ap_distance in zip(anchor_positives, ap_distances):
                    
                    an_distance = distance_matrix[torch.LongTensor(np.array([anchor_positive[0]])), torch.LongTensor(negative_indices)]
                    # an_distance = (np.array([anchor_positive[0]] - negative_indices)**2)
